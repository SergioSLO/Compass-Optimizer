...
JoinQuery parse_sql_file(const std::string &path) {
    ...
    // >>> AQUI declaramos el mapa de alias <<<
    // aliasMap["EMP"] = "Employees" por ejemplo
    std::unordered_map<std::string,std::string> aliasMap;

    // ================== FROM ==================
    {
        size_t start = pos_from + 6; // después de " FROM "
        ...
        // ejemplo: "Employees E" o "Employees AS E" o "Employees"
        auto tokens = split(t, ' ');
        if (tokens.empty()) {
            throw std::runtime_error("No se pudo parsear tabla en FROM");
        }

        std::string realTbl = trim(tokens[0]);
        q.leftTable = realTbl;
        q.tables.push_back(realTbl);

        // alias opcional
        if (tokens.size() >= 2) {
            // soportar "AS alias" o "alias"
            if (to_upper_copy(tokens[1]) == "AS") {
                if (tokens.size() >= 3) {
                    std::string alias = trim(tokens[2]);
    ...
    while (true) {
        ...
        // ----- tabla del JOIN (con posible alias) -----
        {
            size_t start_table = pos_join + 6; // después de " JOIN "
        ...
        // resolver alias en lt/rt si los hay
        auto resolve_alias = [&](std::string &tbl) {
            if (tbl.empty()) return;
            std::string T = to_upper_copy(tbl);
            auto it = aliasMap.find(T);
            if (it != aliasMap.end()) {
                tbl = it->second; // remplaza alias por tabla real
            }
        };
        resolve_alias(lt);
        resolve_alias(rt);

        // Si faltan qualifiers, asumimos: left = tabla anterior, right = tabla recién agregada
        if (lt.empty()) {
    ...
    if (pos_where != std::string::npos) {
        ...
        while (pos < where_block.size()) {
            ...
            // Resolver alias en WHERE
            std::string T = to_upper_copy(t);
            auto itA = aliasMap.find(T);
            if (itA != aliasMap.end()) {
                t = itA->second;
            }
